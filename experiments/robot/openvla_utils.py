...
OPENVLA_IMAGE_SIZE = 384  # Standard image size expected by OpenVLA
...
def get_vla(cfg: Any) -> torch.nn.Module:
    ...
    if _is_native_prismatic_checkpoint_path(ckpt_path):
        vla = load_vla(
            ...
            base_vlm=getattr(cfg, "base_vlm", None),
            llm_checkpoint_path=getattr(cfg, "llm_checkpoint_path", None),
            vjepa_checkpoint_path=getattr(cfg, "vjepa_checkpoint_path", None),
            dino_local_path=getattr(cfg, "dino_local_path", None),
            siglip_local_path=getattr(cfg, "siglip_local_path", None),
            use_aux_head=getattr(cfg, "use_aux_head", None),
            load_visual_token_cosine_head=getattr(cfg, "load_visual_token_cosine_head", False),
        )
        vla.eval()
        vla = vla.to(DEVICE)
        return vla
...
def get_action_head(cfg: Any, llm_dim: int, action_dim: Optional[int] = None) -> Union[L1RegressionActionHead]:
    """
    ...
    """
    action_head_type = getattr(cfg, "action_head_type", None)
    if action_head_type is None:
        action_head_type = "l1" if getattr(cfg, "use_l1_regression", True) else "flow_gr00t"
    ...
    if action_head_type == "l1":
        action_head = L1RegressionActionHead(
            input_dim=llm_dim, 
            hidden_dim=llm_dim, 
            action_dim=ACTION_DIM if action_dim is None else action_dim,
            use_pro_version=cfg.use_pro_version,
        )
    else:
        raise ValueError(f"Unsupported non-native action head type: {action_head_type}")
...
def prepare_images_for_vla(images: List[np.ndarray], cfg: Any) -> List[Image.Image]:
    ...
    """
    processed_images = []
    target_size = getattr(cfg, "policy_image_size", OPENVLA_IMAGE_SIZE)
    ...
    for image in images:
        ...
        # Resize if needed
        if image.shape != (target_size, target_size, 3):
            image = resize_image_for_policy(image, target_size)
...
def get_vla_action(
    ...
    with torch.inference_mode():
        ...
        if not getattr(get_vla_action, "_printed_input_image_debug", False):
            print(
                "[get_vla_action] num_images_in_input="
                f"{cfg.num_images_in_input}, collected_images={len(all_images)}, "
                f"obs_keys={sorted(obs.keys())}"
            )
            get_vla_action._printed_input_image_debug = True
        ...
        if processor is None and hasattr(vla, "llm_backbone"):
            ...
            if not getattr(get_vla_action, "_printed_native_image_input_debug", False):
                native_views = len(image_input) if isinstance(image_input, list) else 1
                print(f"[get_vla_action] native image_input views={native_views}")
                get_vla_action._printed_native_image_input_debug = True
        ...
        if all_images:
            ...
            if not getattr(get_vla_action, "_printed_processor_pixel_values_debug", False):
                print(f"[get_vla_action] processor pixel_values shape={tuple(inputs['pixel_values'].shape)}")
                get_vla_action._printed_processor_pixel_values_debug = True
