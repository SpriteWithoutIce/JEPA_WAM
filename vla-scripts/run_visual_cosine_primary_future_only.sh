...
LOG_FILE="${LOG_DIR}/train_jepa_qwen25_cosine_primary_future_only_$(date +%Y%m%d_%H%M%S).log"
...
if [[ -z "${RUN_ID_NOTE:-}" ]]; then
    if [[ "${VISUAL_TOKEN_COSINE_PROJECTION_TYPE}" == "conv" ]]; then
        if [[ "${VISUAL_TOKEN_COSINE_LAYER_IDX}" == "-1" ]]; then
            RUN_ID_NOTE="visual-cosine-qwen25-primary-future-only-irepa"
        else
            RUN_ID_NOTE="visual-cosine-qwen25-primary-future-only-irepa-layer${VISUAL_TOKEN_COSINE_LAYER_IDX}"
        fi
    else
        if [[ "${VISUAL_TOKEN_COSINE_LAYER_IDX}" == "-1" ]]; then
            RUN_ID_NOTE="visual-cosine-qwen25-primary-future-only"
        else
            RUN_ID_NOTE="visual-cosine-qwen25-primary-future-only-layer${VISUAL_TOKEN_COSINE_LAYER_IDX}"
        fi
    fi
fi
...
CMD=(
    ...
    --vla.visual_token_cosine_target_future_only True
