...
RUN_ID_NOTE="${RUN_ID_NOTE:-aloha-robotwin-50-llm-projector-action-query}"
mkdir -p "${LOG_DIR}"
...
NUM_GPUS="${NUM_GPUS:-8}"
# V-JEPA remains frozen, so retain the original per-device batch size.
...
CPU_MEMORY_LOG_INTERVAL="${CPU_MEMORY_LOG_INTERVAL:-20}"
ROBOTWIN_ALOHA_MOSAIC="${ROBOTWIN_ALOHA_MOSAIC:-True}"
TORCHRUN_BIN="${TORCHRUN_BIN:-/home/linyihan/jepa_copy/bin/torchrun}"

# Conservative LR for pretrained V-JEPA/Qwen/projector; higher LR for fresh heads.
...
CMD=(
  ...
  --vla.rank_shard_dataset_sources True
  --vla.use_lora False
...
{
  echo "Starting RoboTwin ALOHA 50-task LLM/projector/action-query training"
  echo "Log file: ${LOG_FILE}"
  echo "Runs: ${RUNS_DIR}"
  echo "Data mix: ${DATA_MIX} (50 tasks, clean + randomized)"
