...
LOG_FILE="${LOG_DIR}/train_jepa_qwen25_cosine_primary_lerobot_$(date +%Y%m%d_%H%M%S).log"
LEROBOT_DATA="${LEROBOT_DATA:-/home/linyihan/datasets/libero}"
...
if [[ ! -f "${LEROBOT_DATA}/meta/info.json" ]]; then
    echo "LeRobot dataset not found at: ${LEROBOT_DATA}" >&2
    exit 1
fi
...
CMD=(
    ...
    --data_root_dir "${LEROBOT_DATA}"
    --vla.dataset_format lerobot
    --vla.lerobot_normalization_clip_value 1.0
    --vla.lerobot_num_workers 8
    --vla.lerobot_prefetch_factor 3
