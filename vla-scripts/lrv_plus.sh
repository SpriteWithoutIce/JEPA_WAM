...
LOG_FILE="eval_log/libero_plus_${TASK_SUITE}_cat${CATEGORY_FILTER}_gpu${CUDA_ID}_$(date +%Y%m%d_%H%M%S).log"

echo "Evaluating LIBERO-plus with action_head_type=${ACTION_HEAD_TYPE}, num_images_in_input=${NUM_IMAGES_IN_INPUT}, trials=${TRIALS}, categories=${CATEGORY_FILTER}"
