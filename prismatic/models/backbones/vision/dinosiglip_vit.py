...
class DinoSigLIPViTBackbone(VisionBackbone):
    def __init__(
        ...
        dino_pretrained = dino_local_path is None
        self.dino_featurizer: VisionTransformer = timm.create_model(
            self.dino_timm_path_or_url, pretrained=dino_pretrained, num_classes=0, img_size=self.default_image_size
        )
        if dino_local_path is not None:
            self._load_local_weights_into(self.dino_featurizer, dino_local_path)
        self.dino_featurizer.eval()

        siglip_pretrained = siglip_local_path is None
        self.siglip_featurizer: VisionTransformer = timm.create_model(
            self.siglip_timm_path_or_url, pretrained=siglip_pretrained, num_classes=0, img_size=self.default_image_size
        )
        if siglip_local_path is not None:
            self._load_local_weights_into(self.siglip_featurizer, siglip_local_path)
    ...
    @staticmethod
    def _load_local_weights_into(featurizer: VisionTransformer, local_path: str) -> None:
        checkpoint_path = TimmViTBackbone._resolve_local_checkpoint(local_path)
        ...
        missing, unexpected = featurizer.load_state_dict(
            TimmViTBackbone._normalize_state_dict(state_dict),
            strict=False,
        )
    ...
    @staticmethod
    def _forward_multi_image(featurizer: VisionTransformer, pixel_values: torch.Tensor) -> torch.Tensor:
        if pixel_values.dim() == 4:
            return featurizer(pixel_values)
        if pixel_values.dim() != 5:
            raise ValueError(
                f"DinoSigLIPViTBackbone expects 4D or 5D tensors, got shape {tuple(pixel_values.shape)}"
            )
        ...
        flat_patches = featurizer(flat_pixels)
        _, num_patches, embed_dim = flat_patches.shape
        return flat_patches.reshape(batch_size, num_images * num_patches, embed_dim)

    def forward(self, pixel_values: Dict[str, torch.Tensor]) -> torch.Tensor:
        """Runs the transformed image/pixel tensors through each vision backbone, returning concatenated patches."""
        dino_patches = self._forward_multi_image(self.dino_featurizer, pixel_values["dino"])
        siglip_patches = self._forward_multi_image(self.siglip_featurizer, pixel_values["siglip"])
