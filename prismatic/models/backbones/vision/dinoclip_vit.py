...
class DinoCLIPViTBackbone(VisionBackbone):
    ...
    def _forward_multi_image(featurizer: VisionTransformer, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        if pixel_values.dim() != 5:
            raise ValueError(
                f"DinoCLIPViTBackbone expects 4D or 5D tensors, got shape {tuple(pixel_values.shape)}"
            )
    ...
    def forward(self, pixel_values: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        clip_patches = self._forward_multi_image(self.clip_featurizer, pixel_values["clip"])
