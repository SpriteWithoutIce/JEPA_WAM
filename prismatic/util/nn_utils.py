...
def apply_lora_to_linear_layers(module: nn.Module, rank: int, alpha: int, dropout: float = 0.0) -> None:
    """Recursively replace every `nn.Linear` with a frozen base + LoRA residual."""
    ...
    for name, child in list(module.named_children()):
        ...
        if isinstance(child, nn.Linear):
            ...
            continue
        apply_lora_to_linear_layers(child, rank=rank, alpha=alpha, dropout=dropout)
