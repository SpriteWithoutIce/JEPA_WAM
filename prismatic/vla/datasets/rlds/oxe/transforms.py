...
from prismatic.util.rotation_utils import AXIS_ANGLE, ROT6D, axis_angle_to_rot6d_tf, validate_rotation_representation
from prismatic.vla.datasets.rlds.oxe.robotwin_aloha import ROBOTWIN_ALOHA_DATASETS
...
def libero_dataset_transform(
    ...
    *,
    ...
    if rotation_representation == ROT6D:
        action_rot6d = axis_angle_to_rot6d_tf(trajectory["action"][:, 3:6])
        eef_rot6d = axis_angle_to_rot6d_tf(trajectory["observation"]["state"][:, 3:6])
        trajectory["action"] = tf.concat(
            [
                trajectory["action"][:, :3],
                action_rot6d,
...
def aloha_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    # RoboTwin stores language variants as [T, num_variants]. Sample one
    # variant per trajectory so all timesteps share the same instruction.
    language_instruction = trajectory["language_instruction"]
    if language_instruction.shape.rank == 2:
        variants = language_instruction[0]
        variant_index = tf.random.uniform(
            shape=(), maxval=tf.shape(variants)[0], dtype=tf.int32
        )
        selected_instruction = variants[variant_index]
        trajectory["language_instruction"] = tf.repeat(
            selected_instruction[None], tf.shape(trajectory["action"])[0]
        )
    return trajectory
...
OXE_STANDARDIZATION_TRANSFORMS = {
    ...
    **{dataset_name: aloha_dataset_transform for dataset_name in ROBOTWIN_ALOHA_DATASETS},
}
