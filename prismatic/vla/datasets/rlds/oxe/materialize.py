...
from prismatic.util.rotation_utils import AXIS_ANGLE, ROT6D, validate_rotation_representation
...
def make_oxe_dataset_kwargs(
    ...
    if rotation_representation == ROT6D:
        if "libero" not in dataset_name:
            raise ValueError("`rotation_representation=rot6d` is currently implemented only for LIBERO datasets.")
        dataset_kwargs["action_encoding"] = ActionEncoding.EEF_R6
    ...
    if "libero" in dataset_name:
        standardize_fn = partial(standardize_fn, rotation_representation=rotation_representation)
